...
impl EmitterItems {
    pub fn new(jni: &'static JNI) -> Self {
        ...
        let methods = [mn.item_get_desc_id.new_method_node(av, jni, ACC_PUBLIC | ACC_NATIVE).unwrap()];
        ...
        cls.register_natives(&[mn.item_get_desc_id.native(get_desc_id_dyn())]).unwrap();
